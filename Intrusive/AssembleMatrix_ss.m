function [A] = AssembleMatrix_ss(points, edges, triangles, pde, W, time, ord_in, dim, num_spectral,n_ipce)
...
for triangle = triangles
  ...
  for p = midpoints
	...
	integral = feval(pde, u, v, w, du, dv, dw, dx, 0, x, d, time, 1, ord_in, dim, num_spectral,n_ipce) / 3.0;
...
%   d = edge(5);
  ...
  for p = gausspoints
	...
	integral = feval(pde, u, v, w, [0;0], [0;0], dw, 0, ds, x, d, time, 1, ord_in, dim, num_spectral,n_ipce) / 2.0;
	A(nodes(i),nodes(j)) = A(nodes(i),nodes(j)) + integral;
