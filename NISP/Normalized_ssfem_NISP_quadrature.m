...
%%% Some PC coefficients have opposite signs, pdf not very good match,
...
for n_pc = 1:ord_out+1
    ...
    b_NISP = (b.*psi(i,n_pc)*w(i))/sqrt(norm.norm_squared(n_pc));
    
    sum_numerator = sum_numerator + A_NISP\b_NISP;
    
    end
        
    U(:,n_pc) = (sum_numerator);
...
for pc = 1:n_outpce
U_coeff(pc) = U(lo,pc);    
U_pdf = U_pdf + U(lo,pc).*psi_pdf(:,pc) ./sqrt(norm.norm_squared(pc));
