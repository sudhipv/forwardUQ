...
filename_norm = strcat('norm_squared',str5);
...
for ipce=1:n_inpce
% Assemble matrix
A(:,:,ipce) = AssembleMatrix_ss(p, e, t, 'Poisson_ss', [], 0, s_spectral,ipce,file);
end
...
for k = 1: n_outpce
    ...
    A_jk((k-1)*n_A+1:(k-1)*n_A+n_A, (j-1)*n_A+1:(j-1)*n_A+n_A)...
        = A_jk((k-1)*n_A+1:(k-1)*n_A+n_A, (j-1)*n_A+1:(j-1)*n_A+n_A) + Cijk.moment_ND(i,j,k) * A(:,:,i);
...
% Norm Squared
%%% Multiplication Tensor
filename = strcat('norm_squared',str5);
norm = load(filename);
norm.norm_squared;
...
for k = 1: n_outpce
    ...
    b_k(((k-1)*size(b,1)+1:(k-1)*size(b,1)+size(b,1)),1) = b(1:size(b,1),1)* exp_pce(k)/norm.norm_squared(k);
...
for pp = 1:n_outpce
U_pdf(:,1) = U_pdf + U(lo+(pp-1)*index_coeff,1).*psi_pdf(:,pp);
end
