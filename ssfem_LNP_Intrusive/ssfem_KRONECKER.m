...
%%%%%A_jk_Kron = sum (i= 0, n_outpce) {G_jk ** Ai}

 G_jk = zeros(n_outpce,n_outpce,n_inpce);
 ...
 for i= 1: n_inpce
             ...
             G_jk(:,:,i) = cd (i ) * Cijk.moment_PsiNorm(i,:,:);
            
 end
 
 A_jk_Kron = zeros(size(A,1)*n_outpce);
 

 
 G_jk_sum = zeros(n_outpce,n_outpce);
 
 tic
 
 for i = 1:n_inpce
     
       G_jk_sum = G_jk_sum + G_jk(:,:,i);
     
%       A_jk_Kron = A_jk_Kron + kron(G_jk(:,:,i),A(:,:,i));
      
      
 end
 
 A_jk_Kron = kron(G_jk_sum,A(:,:,1));
